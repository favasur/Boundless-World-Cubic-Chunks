...
public class AnnotatedMixinElementHandlerAccessor extends AnnotatedMixinElementHandler implements IMixinContext {
         ...
         if (targetName == null) {
            ...
            for (TypeHandle target : this.mixin.getTargets()) {
               try {
                  ...
                  elem.printMessage(this.ap, Kind.ERROR, var6.getMessage());
                  continue;
               }

               if (elem.getAccessorType() == AccessorInfo.AccessorType.OBJECT_FACTORY) {
                  this.registerFactoryForTarget((AnnotatedMixinElementHandlerAccessor.AnnotatedElementInvoker)elem, target);
   ...
   private void registerFactoryForTarget(AnnotatedMixinElementHandlerAccessor.AnnotatedElementInvoker elem, TypeHandle target) {
         ...
         if (equivalency.type != TypeUtils.Equivalency.EQUIVALENT_BUT_RAW || equivalency.rawType != 1) {
            if (equivalency.type == TypeUtils.Equivalency.BOUNDS_MISMATCH) {
               elem.printMessage(
                  this.ap,
                  Kind.ERROR,
                  "Invalid Factory @Invoker return type, generic type arguments of "
                     + target.getType()
               ...
               elem.printMessage(
                  this.ap, Kind.ERROR, "Invalid Factory @Invoker return type, expected " + target.getType() + " but found " + elem.getReturnType()
               );
               return;
            }
         }
