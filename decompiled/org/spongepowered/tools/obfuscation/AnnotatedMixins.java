...
final class AnnotatedMixins implements IMixinAnnotationProcessor, ITokenProvider, ITypeHandleProvider, IJavadocProvider {
   ...
   private AnnotatedMixins(ProcessingEnvironment processingEnv) {
      ...
      this.printMessage(Kind.NOTE, "SpongePowered MIXIN Annotation Processor Version=0.8.1" + pluginVersionString);
   ...
   private String checkPluginVersion(String version) {
      if (version == null) {
         ...
         if (pluginVersion.compareTo(recommendedVersion) < 0) {
            this.printMessage(
               Kind.WARNING, String.format("MixinGradle version %s is out of date. Update to the recommended version %s", pluginVersion, recommendedVersion)
            );
         }
   ...
   private IMixinAnnotationProcessor.CompilerEnvironment detectEnvironment(ProcessingEnvironment processingEnv) {
      return processingEnv.getClass().getName().contains("jdt")
         ? IMixinAnnotationProcessor.CompilerEnvironment.JDT
         : IMixinAnnotationProcessor.CompilerEnvironment.JAVAC;
   }
   ...
   private static boolean shouldSuppress(Element element, SuppressedBy suppressedBy) {
      if (element != null && suppressedBy != null) {
         return AnnotationHandle.of(element, SuppressWarnings.class).getList().contains(suppressedBy.getToken())
            ? true
            : shouldSuppress(element.getEnclosingElement(), suppressedBy);
      } else {
         return false;
      }
   }
   ...
   @Override
   public void printMessage(Kind kind, CharSequence msg, Element element, AnnotationMirror annotation, AnnotationValue value, SuppressedBy suppressedBy) {
      if (kind != Kind.WARNING || !shouldSuppress(element, suppressedBy)) {
         this.processingEnv.getMessager().printMessage(kind, msg, element, annotation, value);
      }
   }
