      ...
      for (Element elem : roundEnv.getElementsAnnotatedWith(injectorClass)) {
         ...
         if (!(parent instanceof TypeElement)) {
            throw new IllegalStateException(
               "@" + injectorClass.getSimpleName() + " element has unexpected parent with type " + TypeUtils.getElementType(parent)
            );
         }
         ...
         if (elem.getKind() == ElementKind.METHOD) {
            ...
            this.mixins
               .printMessage(Kind.WARNING, "Found an @" + injectorClass.getSimpleName() + " annotation on an element which is not a method: " + elem.toString());
         }
      }
   }
}
