...
public class MethodHandle extends MemberHandle<MappingMethod> {
   ...
   public MappingMethod asMapping(boolean includeOwner) {
      if (includeOwner) {
         return (MappingMethod)(this.ownerHandle != null
            ? new MappingMethodResolvable(this.ownerHandle, this.getName(), this.getDesc())
            : new MappingMethod(this.getOwner(), this.getName(), this.getDesc()));
      } else {
