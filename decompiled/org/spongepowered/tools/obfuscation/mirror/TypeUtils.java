...
public abstract class TypeUtils {
   ...
   public static TypeUtils.EquivalencyResult isEquivalentType(ProcessingEnvironment processingEnv, TypeMirror t1, TypeMirror t2) {
      if (t1 != null && t2 != null) {
               ...
               if (!processingEnv.getTypeUtils().isSameType(rawT1, rawT2)) {
                  ...
                  if (argsT1.size() != argsT2.size()) {
                     if (argsT1.size() == 0) {
                        ...
                        return argsT2.size() == 0
                           ? TypeUtils.EquivalencyResult.equivalentButRaw(2)
                           : TypeUtils.EquivalencyResult.notEquivalent(
                              "Mismatched generic argument counts %s<[%d]> and %s<[%d]>", rawT1, argsT1.size(), rawT2, argsT2.size()
                           );
                     }
                  } else {
                     for (int arg = 0; arg < argsT1.size(); arg++) {
                        ...
                        if (isEquivalentType(processingEnv, argT1, argT2).type != TypeUtils.Equivalency.EQUIVALENT) {
                           return TypeUtils.EquivalencyResult.boundsMismatch(
                              "Generic bounds mismatch between %s and %s", describeGenericBound(argT1), describeGenericBound(argT2)
                           );
                        }
                     }
   ...
   public static class EquivalencyResult {
      ...
      static TypeUtils.EquivalencyResult notEquivalent(String format, Object... args) {
         return new TypeUtils.EquivalencyResult(TypeUtils.Equivalency.NOT_EQUIVALENT, String.format(format, args), 0);
      }

      static TypeUtils.EquivalencyResult boundsMismatch(String format, Object... args) {
         return new TypeUtils.EquivalencyResult(TypeUtils.Equivalency.BOUNDS_MISMATCH, String.format(format, args), 0);
      }

      static TypeUtils.EquivalencyResult equivalentButRaw(int rawType) {
         return new TypeUtils.EquivalencyResult(TypeUtils.Equivalency.EQUIVALENT_BUT_RAW, String.format("Type %d is raw", rawType), rawType);
      }
   }
}
