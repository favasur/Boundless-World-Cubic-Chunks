...
public final class MethodSlice {
   ...
   public ReadOnlyInsnList getSlice(MethodNode method) {
      ...
      if (start > end) {
         ...
         return (ReadOnlyInsnList)(start == 0 && end == max
            ? new ReadOnlyInsnList(method.instructions)
            : new MethodSlice.InsnListSlice(method.instructions, start, end));
      }
   }
   ...
   public static MethodSlice parse(ISliceContext info, AnnotationNode node) {
      String id = Annotations.getValue(node, "id");
      AnnotationNode from = Annotations.getValue(node, "from");
      AnnotationNode to = Annotations.getValue(node, "to");
