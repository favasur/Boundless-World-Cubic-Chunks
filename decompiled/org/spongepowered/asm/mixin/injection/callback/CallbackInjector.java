...
public class CallbackInjector extends Injector {
            ...
            if (existingId != null && !existingId.equals(id)) {
               Injector.logger
                  .warn(
                     ...
                     new Object[]{Bytecode.getOpcodeName(node), target.toString(), id, this.info, existingId}
                  );
               break;
            }
   ...
   private void inject(CallbackInjector.Callback callback) {
      if (this.localCapture.isPrintLocals()) {
         ...
         } else if (this.info.getTargets().size() <= 1) {
   ...
   private String generateBadLVTMessage(CallbackInjector.Callback callback) {
      ...
      return expected.equals(found)
         ? String.format("Invalid descriptor on %s! Expected %s but found %s", this.info, callback.getDescriptor(), this.methodNode.desc)
         : String.format(
