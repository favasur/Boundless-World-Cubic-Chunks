...
public class BeforeInvoke extends InjectionPoint {
   ...
   public BeforeInvoke(InjectionPointData data) {
      ...
      this.allowPermissive = this.context.getOption(MixinEnvironment.Option.REFMAP_REMAP)
         && this.context.getOption(MixinEnvironment.Option.REFMAP_REMAP_ALLOW_PERMISSIVE)
   ...
   public boolean find(String desc, InsnList insns, Collection<AbstractInsnNode> nodes) {
      ...
      if (!found && hasDescriptor && this.allowPermissive) {
         this.logger
            .warn(
               "STRICT match for {} using \"{}\" in {} returned 0 results, attempting permissive search. To inhibit permissive search set mixin.env.allowPermissiveMatch=false",
               new Object[]{this.className, this.target, this.context}
            );
   ...
   protected boolean find(String desc, InsnList insns, Collection<AbstractInsnNode> nodes, ITargetSelector selector, BeforeInvoke.SearchType searchType) {
      if (selector == null) {
         return false;
      } else {
         ITargetSelector target = searchType == BeforeInvoke.SearchType.PERMISSIVE ? selector.configure("permissive") : selector;
         int ordinal = 0;
         int found = 0;
         ...
         if (searchType == BeforeInvoke.SearchType.PERMISSIVE && found > 1) {
            this.logger
               .warn(
                  "A permissive match for {} using \"{}\" in {} matched {} instructions, this may cause unexpected behaviour. To inhibit permissive search set mixin.env.allowPermissiveMatch=false",
                  new Object[]{this.className, selector, this.context, found}
               );
         }
