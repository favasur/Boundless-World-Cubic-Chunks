   ...
   protected void injectAtInvoke(Target target, InjectionNodes.InjectionNode node) {
      ...
      if (args.length == 0) {
         throw new InvalidInjectionException(
            this.info, "@ModifyArgs injector " + this + " targets a method invocation " + targetMethod.name + targetMethod.desc + " with no arguments!"
         );
      } else {
   ...
   private boolean verifyTarget(Target target) {
      ...
      if (!this.methodNode.desc.equals(shortDesc)) {
         ...
         if (this.methodNode.desc.equals(longDesc)) {
            ...
            throw new InvalidInjectionException(
               this.info, "@ModifyArgs injector " + this + " has an invalid signature " + this.methodNode.desc + ", expected " + shortDesc + " or " + longDesc
            );
         }
      } else {
         return false;
