...
public class ModifyConstantInjector extends RedirectInjector {
         ...
         if (node.isReplaced()) {
            ...
            if (targetNode instanceof TypeInsnNode) {
               ...
               throw new InvalidInjectionException(
                  this.info, String.format("%s annotation is targetting an invalid insn in %s in %s", this.annotationType, target, this)
               );
            }
         }
      }
   ...
   private void injectTypeConstantModifier(Target target, TypeInsnNode typeNode) {
      ...
      if (opcode != 193) {
         throw new InvalidInjectionException(
            this.info, String.format("%s annotation does not support %s insn in %s in %s", this.annotationType, Bytecode.getOpcodeName(opcode), target, this)
         );
      } else {
   ...
   private void injectExpandedConstantModifier(Target target, JumpInsnNode jumpNode) {
      ...
      if (opcode >= 155 && opcode <= 158) {
         ...
         throw new InvalidInjectionException(
            this.info,
            String.format("%s annotation selected an invalid opcode %s in %s in %s", this.annotationType, Bytecode.getOpcodeName(opcode), target, this)
         );
      }
   }
      ...
      if (pop != null) {
         if (pop instanceof FieldInsnNode) {
            ...
            this.checkNarrowing(
               ...
               target.indexOf(pop),
               String.format(
                  "%s %s %s.%s", Bytecode.getOpcodeName(pop), SignaturePrinter.getTypeName(fieldType, false), fieldNode.owner.replace('/', '.'), fieldNode.name
               )
            );
         } else if (pop.getOpcode() == 172) {
            this.checkNarrowing(
               target, constNode, constantType, target.returnType, target.indexOf(pop), "RETURN " + SignaturePrinter.getTypeName(target.returnType, false)
            );
            ...
            if (localVar != null && localVar.desc != null) {
               ...
               this.checkNarrowing(
                  ...
                  localType,
                  target.indexOf(pop),
                  String.format("ISTORE[var=%d] %s %s", var, SignaturePrinter.getTypeName(localType, false), name)
               );
            }
         }
      }
   ...
   private void checkNarrowing(Target target, AbstractInsnNode constNode, Type constantType, Type type, int index, String description) {
      ...
      if (toSort < fromSort) {
         ...
         Injector.logger
            .log(
               level,
               "Narrowing conversion of <{}> to <{}> in {} target {} at opcode {} ({}){}",
               new Object[]{fromType, toType, this.info, target, index, description, message}
            );
      }
   }
}
