...
public class ModifyArgInjector extends InvokeInjector {
   ...
   private void injectMultiArgHandler(Target target, Target.Extension extraLocals, Type[] args, int argIndex, InsnList insns) {
      if (!Arrays.equals((Object[])args, (Object[])this.methodArgs)) {
         throw new InvalidInjectionException(
            this.info,
            "@ModifyArg method "
               + this
               + " targets a method with an invalid signature "
               + Bytecode.getDescriptor(args)
               + ", expected "
   ...
   protected int findArgIndex(Target target, Type[] args) {
      if (this.index > -1) {
         ...
         if (argIndex == -1) {
            throw new InvalidInjectionException(
               this.info, "Could not find arg matching type " + this.returnType + " for @ModifyArg target " + target + " in " + this
            );
         } else {
            return argIndex;
         }
