...
public final class ArgsClassGenerator implements IClassGenerator {
   ...
   public ISyntheticClassInfo getArgsClass(String desc, IMixinInfo mixin) {
      ...
      ArgsClassGenerator.ArgsClassInfo info = this.descToClass.get(voidDesc);
      if (info == null) {
         ...
         info = new ArgsClassGenerator.ArgsClassInfo(mixin, name, voidDesc);
   ...
   public boolean generate(String name, ClassNode classNode) {
      ArgsClassGenerator.ArgsClassInfo info = this.nameToClass.get(name);
      if (info == null) {
         return false;
      } else {
         if (info.loaded > 0) {
   ...
   private void generateCtor(ArgsClassGenerator.ArgsClassInfo info, ClassVisitor writer) {
   ...
   private void generateToString(ArgsClassGenerator.ArgsClassInfo info, ClassVisitor writer) {
   ...
   private void generateFactory(ArgsClassGenerator.ArgsClassInfo info, ClassVisitor writer) {
   ...
   private void generateGetters(ArgsClassGenerator.ArgsClassInfo info, ClassVisitor writer) {
      byte argIndex = 0;
   ...
   private void generateSetters(ArgsClassGenerator.ArgsClassInfo info, ClassVisitor writer) {
      this.generateIndexedSetter(info, writer);
      this.generateMultiSetter(info, writer);
   }

   private void generateIndexedSetter(ArgsClassGenerator.ArgsClassInfo info, ClassVisitor writer) {
   ...
   private void generateMultiSetter(ArgsClassGenerator.ArgsClassInfo info, ClassVisitor writer) {
