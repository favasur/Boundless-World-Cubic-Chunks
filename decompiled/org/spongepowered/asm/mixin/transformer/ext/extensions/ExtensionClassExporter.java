...
public class ExtensionClassExporter implements IExtension {
   ...
   public ExtensionClassExporter(MixinEnvironment env) {
      ...
      try {
         ...
         logger.debug("Error cleaning class output directory: {}", new Object[]{var3.getMessage()});
      }
   }
   ...
   private IDecompiler initDecompiler(MixinEnvironment env, File outputPath) {
      if (!env.getOption(MixinEnvironment.Option.DEBUG_EXPORT_DECOMPILE)) {
         ...
         try {
            ...
            logger.info(
               "Fernflower decompiler was successfully initialised, exported classes will be decompiled{}", new Object[]{as ? " in a separate thread" : ""}
            );
            return decompiler;
         } catch (Throwable var8) {
            logger.info(
               "Fernflower could not be loaded, exported classes will not be decompiled. {}: {}",
               new Object[]{var8.getClass().getSimpleName(), var8.getMessage()}
            );
            return null;
         }
      }
