...
class SyntheticClassRegistry implements ISyntheticClassRegistry {
   ...
   public ISyntheticClassInfo findSyntheticClass(String name) {
      return name == null ? null : this.classes.get(name.replace('.', '/'));
   }
      ...
      if (info != null) {
         if (info != sci) {
            throw new MixinError(
               "Synthetic class with name " + name + " was already registered by " + info.getMixin() + ". Duplicate being registered by " + sci.getMixin()
            );
         }
      } else {
