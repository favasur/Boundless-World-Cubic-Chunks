...
public final class Bytecode {
   ...
   public static Bytecode.DelegateInitialiser findDelegateInit(MethodNode ctor, String superName, String ownerName) {
               ...
               if ("<init>".equals(methodNode.name)) {
                  if (news <= 0) {
                     ...
                     if (isSuper || methodNode.owner.equals(ownerName)) {
                        return new Bytecode.DelegateInitialiser(methodNode, isSuper);
                     }
                  } else {
                     news--;
                  }
   ...
   public static void textify(MethodNode methodNode, OutputStream out) {
      ...
      MethodVisitor mv = trace.visitMethod(
         methodNode.access, methodNode.name, methodNode.desc, methodNode.signature, methodNode.exceptions.toArray(new String[0])
      );
         ...
         if (node instanceof JumpInsnNode) {
            ...
            out = out
               + String.format(
                  ...
                  idc.name,
                  idc.desc,
                  getOpcodeName(idc.bsm.getTag(), "H_GETFIELD", 1),
                  idc.bsm.getOwner(),
                  idc.bsm.getName(),
                  idc.bsm.getDesc()
               );
   ...
   public static String generateDescriptor(Type returnType, Type... args) {
      return generateDescriptor(returnType, args);
   }
