         ...
         case BAD_LOAD:
            ...
            for (int index = 0; ia.hasNext() && ib.hasNext(); index++) {
               ...
               if (na instanceof VarInsnNode && nb instanceof VarInsnNode) {
                  ...
                  Type tax = va.var > 0 ? argsa[va.var - 1] : target;
                  Type tbx = vb.var > 0 ? argsb[vb.var - 1] : target;
                  printer.kv("Target " + index, "%8s %-2d %s", Bytecode.getOpcodeName(va), va.var, tax);
                  printer.kv("Incoming " + index, "%8s %-2d %s", Bytecode.getOpcodeName(vb), vb.var, tbx);
                  if (tax.equals(tbx)) {
                     printer.kv("", "Types match: %s", tax);
                  } else if (tax.getSort() != tbx.getSort()) {
                     printer.kv("", "Types are incompatible");
                  } else if (tax.getSort() == 10) {
                     ClassInfo superClass = ClassInfo.getCommonSuperClassOrInterface(tax, tbx);
