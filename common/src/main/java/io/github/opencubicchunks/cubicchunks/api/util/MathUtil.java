...
/**
 * General math helpers used by world generation and cube logic.
 * Ported from 1.12.2 Cubic Chunks.
 */
// @Original: 1.12.2:io.github.opencubicchunks.cubicchunks.api.util.MathUtil
public final class MathUtil {
    ...
    public static double gaussianProbabilityDensity(double x, double mean, double stdDev) {
        return Math.exp(-(x - mean) * (x - mean) / (2.0 * stdDev * stdDev))
                / (Math.sqrt(Math.PI * 2) * stdDev);
    }
